...
The Windows core library for this language is available at [AELibrary](https://github.com/Aethel-Systems/AELibrary).
...
Windows library path note: set `[Environment]::SetEnvironmentVariable("AELibraryPATH", "C:\Your\AELibrary\Path", "User")` before using `--lib`.
...
*   **Target Output**: The binaries generated by this toolchain are typically in **Aethelium Native**, **x86 Machine Code**, or **UEFI PE** formats, The latest source code now supports EXE launches using ntdll.dll, but it cannot run in Unix-like environments such as Linux/macOS.
*   **Cross-Compilation**: The toolchain provided in this repository is essentially a cross-compiler running on a host machine.
*   **Reject POSIX**: Aethelium will never bootstrap on a host machine. It will only rewrite itself in the future when new, Unix-free, POSIX-free, and C-free systems emerge.
